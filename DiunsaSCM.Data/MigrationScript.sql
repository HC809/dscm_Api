...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventItem]') AND [c].[name] = N'WebSiteDescription');
IF @var33 IS NOT NULL EXEC(N'ALTER TABLE [InventItem] DROP CONSTRAINT [' + @var33 + '];');
ALTER TABLE [InventItem] ALTER COLUMN [WebSiteDescription] nvarchar(max) NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210417231214_NAVFieldsInItemFix', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'BarcodeSetupId');
IF @var34 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var34 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [BarcodeSetupId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'ERPRecId');
IF @var35 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var35 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [ERPRecId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'InventColorId');
IF @var36 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var36 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [InventColorId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'InventSizeId');
IF @var37 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var37 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [InventSizeId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'ItemId');
IF @var38 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var38 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [ItemId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'ItemName');
IF @var39 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var39 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [ItemName];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'NameAlias');
IF @var40 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var40 + '];');
ALTER TABLE [ItemBarcode] DROP COLUMN [NameAlias];

GO

ALTER TABLE [ItemBarcode] ADD [InventDimId] bigint NULL;

GO

ALTER TABLE [ItemBarcode] ADD [InventItemId] bigint NOT NULL DEFAULT CAST(0 AS bigint);

GO

CREATE INDEX [IX_ItemBarcode_InventDimId] ON [ItemBarcode] ([InventDimId]);

GO

CREATE INDEX [IX_ItemBarcode_InventItemId] ON [ItemBarcode] ([InventItemId]);

GO

ALTER TABLE [ItemBarcode] ADD CONSTRAINT [FK_ItemBarcode_InventDim_InventDimId] FOREIGN KEY ([InventDimId]) REFERENCES [InventDim] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [ItemBarcode] ADD CONSTRAINT [FK_ItemBarcode_InventItem_InventItemId] FOREIGN KEY ([InventItemId]) REFERENCES [InventItem] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210419024153_BarcodeFields', N'3.1.7');

GO

CREATE SEQUENCE [InventItems] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderShipmentHeader]') AND [c].[name] = N'Code');
IF @var41 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderShipmentHeader] DROP CONSTRAINT [' + @var41 + '];');
ALTER TABLE [PurchOrderShipmentHeader] ALTER COLUMN [Code] nvarchar(max) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventItem]') AND [c].[name] = N'Code');
IF @var42 IS NOT NULL EXEC(N'ALTER TABLE [InventItem] DROP CONSTRAINT [' + @var42 + '];');
ALTER TABLE [InventItem] ALTER COLUMN [Code] nvarchar(max) NULL;
ALTER TABLE [InventItem] ADD DEFAULT (Format(NEXT VALUE FOR dbo.InventItems,'D8')) FOR [Code];

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210419191514_InventItem_Sequence', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderHeader]') AND [c].[name] = N'PurchId');
IF @var43 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderHeader] DROP CONSTRAINT [' + @var43 + '];');
ALTER TABLE [PurchOrderHeader] DROP COLUMN [PurchId];

GO

CREATE SEQUENCE [PurchOrders] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;

GO

CREATE SEQUENCE [PurchOrderShipments] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;

GO

CREATE SEQUENCE [PurchQuotations] START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;

GO

CREATE SEQUENCE [Vendors] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Vendor]') AND [c].[name] = N'Code');
IF @var44 IS NOT NULL EXEC(N'ALTER TABLE [Vendor] DROP CONSTRAINT [' + @var44 + '];');
ALTER TABLE [Vendor] ALTER COLUMN [Code] nvarchar(max) NULL;
ALTER TABLE [Vendor] ADD DEFAULT ('P' + Format(NEXT VALUE FOR dbo.Vendors,'D8')) FOR [Code];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotation]') AND [c].[name] = N'Code');
IF @var45 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotation] DROP CONSTRAINT [' + @var45 + '];');
ALTER TABLE [PurchQuotation] ALTER COLUMN [Code] nvarchar(max) NULL;
ALTER TABLE [PurchQuotation] ADD DEFAULT ('RQ' + Format(NEXT VALUE FOR dbo.PurchQuotations,'D8')) FOR [Code];
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderShipmentHeader]') AND [c].[name] = N'Code');
IF @var46 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderShipmentHeader] DROP CONSTRAINT [' + @var46 + '];');
ALTER TABLE [PurchOrderShipmentHeader] ALTER COLUMN [Code] nvarchar(max) NULL;
ALTER TABLE [PurchOrderShipmentHeader] ADD DEFAULT ('EC' + Format(NEXT VALUE FOR dbo.PurchOrderShipments,'D8')) FOR [Code];

GO

ALTER TABLE [PurchOrderHeader] ADD [Code] nvarchar(max) NULL DEFAULT ('OC' + Format(NEXT VALUE FOR dbo.PurchOrders,'D8'));

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210420013003_InventItem_Sequence_OC_P_EC_RQ', N'3.1.7');

GO

ALTER TABLE [PurchQuotation] ADD [VendorId] bigint NULL;

GO

CREATE INDEX [IX_PurchQuotation_VendorId] ON [PurchQuotation] ([VendorId]);

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_Vendor_VendorId] FOREIGN KEY ([VendorId]) REFERENCES [Vendor] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210420020511_VendorIdInPurchQuotation', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'ColorCode');
IF @var47 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var47 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [ColorCode];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'ColorDescription');
IF @var48 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var48 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [ColorDescription];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'InventItemCode');
IF @var49 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var49 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [InventItemCode];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'InventItemDescription');
IF @var50 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var50 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [InventItemDescription];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'ItemBarcodeCode');
IF @var51 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var51 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [ItemBarcodeCode];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'ItemBarcodeDescription');
IF @var52 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var52 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [ItemBarcodeDescription];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'SizeCode');
IF @var53 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var53 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [SizeCode];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLine]') AND [c].[name] = N'SizeDescription');
IF @var54 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLine] DROP CONSTRAINT [' + @var54 + '];');
ALTER TABLE [PurchQuotationLine] DROP COLUMN [SizeDescription];

GO

ALTER TABLE [SalesPriceDefinitionLine] ADD [EstimatedCost] decimal(18,2) NOT NULL DEFAULT 0.0;

GO

ALTER TABLE [InventItem] ADD [EstimatedCost] decimal(18,2) NOT NULL DEFAULT 0.0;

GO

ALTER TABLE [InventItem] ADD [EstimatedCostDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210422172917_EstimatedCostinInventItem', N'3.1.7');

GO

ALTER TABLE [PurchQuotation] ADD [PurchQuotationStatus] int NOT NULL DEFAULT 0;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210424014943_PurchQuotationStatusInPurchQuotation', N'3.1.7');
...
ALTER TABLE [PurchOrderHeader] DROP CONSTRAINT [PK_PurchOrderHeader];

GO

ALTER TABLE [PurchOrderDetail] DROP CONSTRAINT [PK_PurchOrderDetail];

GO

ALTER TABLE [PurchOrderHeader] ADD [PurchOrderHeaderId] bigint NOT NULL IDENTITY;

GO

ALTER TABLE [PurchOrderDetail] ADD [PurchOrderDetailId] bigint NOT NULL IDENTITY;

GO

ALTER TABLE [PurchOrderHeader] ADD CONSTRAINT [PK_PurchOrderHeader] PRIMARY KEY ([PurchOrderHeaderId]);

GO

ALTER TABLE [PurchOrderDetail] ADD CONSTRAINT [PK_PurchOrderDetail] PRIMARY KEY ([PurchOrderDetailId]);

GO

ALTER TABLE [PurchOrderDetail] ADD CONSTRAINT [FK_PurchOrderDetail_PurchOrderHeader_PurchOrderHeaderId] FOREIGN KEY ([PurchOrderHeaderId]) REFERENCES [PurchOrderHeader] ([PurchOrderHeaderId]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchOrderShipmentDetail] ADD CONSTRAINT [FK_PurchOrderShipmentDetail_PurchOrderDetail_PurchOrderDetailId] FOREIGN KEY ([PurchOrderDetailId]) REFERENCES [PurchOrderDetail] ([PurchOrderDetailId]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchOrderShipmentHeader] ADD CONSTRAINT [FK_PurchOrderShipmentHeader_PurchOrderHeader_PurchOrderHeaderId] FOREIGN KEY ([PurchOrderHeaderId]) REFERENCES [PurchOrderHeader] ([PurchOrderHeaderId]) ON DELETE NO ACTION;

GO

ALTER TABLE [ShipmentContainerDetail] ADD CONSTRAINT [FK_ShipmentContainerDetail_PurchOrderDetail_PurchOrderDetailId] FOREIGN KEY ([PurchOrderDetailId]) REFERENCES [PurchOrderDetail] ([PurchOrderDetailId]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210424022002_IdentityInPurchOrderHeader', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderHeader]') AND [c].[name] = N'Id');
IF @var55 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderHeader] DROP CONSTRAINT [' + @var55 + '];');
ALTER TABLE [PurchOrderHeader] DROP COLUMN [Id];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderDetail]') AND [c].[name] = N'Id');
IF @var56 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderDetail] DROP CONSTRAINT [' + @var56 + '];');
ALTER TABLE [PurchOrderDetail] DROP COLUMN [Id];

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210424022328_RemoveOldInPurchOrderHeader', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderHeader]') AND [c].[name] = N'PurchOrderHeaderId');
IF @var57 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderHeader] DROP CONSTRAINT [' + @var57 + '];');
ALTER TABLE [PurchOrderHeader] DROP COLUMN [PurchOrderHeaderId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderDetail]') AND [c].[name] = N'PurchOrderDetailId');
IF @var58 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderDetail] DROP CONSTRAINT [' + @var58 + '];');
ALTER TABLE [PurchOrderDetail] DROP COLUMN [PurchOrderDetailId];

GO

ALTER TABLE [PurchOrderHeader] ADD [Id] bigint NOT NULL IDENTITY;

GO

ALTER TABLE [PurchOrderDetail] ADD [Id] bigint NOT NULL IDENTITY;

GO

ALTER TABLE [PurchOrderHeader] ADD CONSTRAINT [PK_PurchOrderHeader] PRIMARY KEY ([Id]);

GO

ALTER TABLE [PurchOrderDetail] ADD CONSTRAINT [PK_PurchOrderDetail] PRIMARY KEY ([Id]);
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210424022439_RemameIdInPurchOrderHeader', N'3.1.7');

GO

ALTER TABLE [PurchQuotation] ADD [CurrencyId] bigint NULL;

GO

ALTER TABLE [PurchQuotation] ADD [PurchOrderHeaderId] bigint NULL;

GO

ALTER TABLE [PurchQuotation] ADD [PurchPaymentConditionId] bigint NULL;

GO

ALTER TABLE [PurchOrderHeader] ADD [PurchPaymentConditionCode] nvarchar(max) NULL;
...
CREATE TABLE [PurchPaymentCondition] (
    [Id] bigint NOT NULL IDENTITY,
    [Code] nvarchar(max) NULL,
    [Description] nvarchar(max) NULL,
    CONSTRAINT [PK_PurchPaymentCondition] PRIMARY KEY ([Id])
);

GO

CREATE INDEX [IX_PurchQuotation_CurrencyId] ON [PurchQuotation] ([CurrencyId]);

GO

CREATE INDEX [IX_PurchQuotation_PurchOrderHeaderId] ON [PurchQuotation] ([PurchOrderHeaderId]);

GO

CREATE INDEX [IX_PurchQuotation_PurchPaymentConditionId] ON [PurchQuotation] ([PurchPaymentConditionId]);

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_Currency_CurrencyId] FOREIGN KEY ([CurrencyId]) REFERENCES [Currency] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_PurchOrderHeader_PurchOrderHeaderId] FOREIGN KEY ([PurchOrderHeaderId]) REFERENCES [PurchOrderHeader] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_PurchPaymentCondition_PurchPaymentConditionId] FOREIGN KEY ([PurchPaymentConditionId]) REFERENCES [PurchPaymentCondition] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210427023759_PurchQuotationFields', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventItem]') AND [c].[name] = N'NameAlias');
IF @var59 IS NOT NULL EXEC(N'ALTER TABLE [InventItem] DROP CONSTRAINT [' + @var59 + '];');
ALTER TABLE [InventItem] ALTER COLUMN [NameAlias] nvarchar(450) NULL;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventItem]') AND [c].[name] = N'Code');
IF @var60 IS NOT NULL EXEC(N'ALTER TABLE [InventItem] DROP CONSTRAINT [' + @var60 + '];');
ALTER TABLE [InventItem] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [InventItem] ADD DEFAULT (Format(NEXT VALUE FOR dbo.InventItems,'D8')) FOR [Code];

GO

CREATE INDEX [IX_InventItem_Code] ON [InventItem] ([Code]);

GO

CREATE INDEX [IX_InventItem_NameAlias] ON [InventItem] ([NameAlias]);

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210428145924_Indexes', N'3.1.7');

GO

CREATE INDEX [IX_InventItem_Code_NameAlias] ON [InventItem] ([Code], [NameAlias]);

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210428150626_Indexes_Multiple', N'3.1.7');

GO

ALTER TABLE [PurchQuotationLine] ADD [LineNum] int NOT NULL DEFAULT 0;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210428230658_LineNumInPurchQuotationLine', N'3.1.7');

GO

ALTER TABLE [InventItem] ADD [ItemSeasonalCategoryId] bigint NULL;

GO

CREATE TABLE [ItemSeasonalCategory] (
    [Id] bigint NOT NULL IDENTITY,
    [Code] nvarchar(max) NULL,
    [Description] nvarchar(max) NULL,
    CONSTRAINT [PK_ItemSeasonalCategory] PRIMARY KEY ([Id])
);

GO

CREATE INDEX [IX_InventItem_ItemSeasonalCategoryId] ON [InventItem] ([ItemSeasonalCategoryId]);

GO

ALTER TABLE [InventItem] ADD CONSTRAINT [FK_InventItem_ItemSeasonalCategory_ItemSeasonalCategoryId] FOREIGN KEY ([ItemSeasonalCategoryId]) REFERENCES [ItemSeasonalCategory] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210429104146_ItemSeasonalCategory', N'3.1.7');

GO

ALTER TABLE [PurchQuotation] ADD [DeliveryDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchQuotation] ADD [ShipmentDateConfirmed] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchQuotation] ADD [ShipmentDateRequested] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchQuotation] ADD [WarehouseId] bigint NULL;

GO

ALTER TABLE [PurchOrderHeader] ADD [DeliveryDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchOrderHeader] ADD [ShipmentDateConfirmed] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchOrderHeader] ADD [ShipmentDateRequested] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchOrderHeader] ADD [WarehouseCode] nvarchar(max) NULL;

GO

ALTER TABLE [PurchOrderHeader] ADD [WarehouseDescription] nvarchar(max) NULL;

GO

ALTER TABLE [InventItem] ADD [UME] int NOT NULL DEFAULT 0;
...
CREATE INDEX [IX_PurchQuotation_WarehouseId] ON [PurchQuotation] ([WarehouseId]);

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_Warehouse_WarehouseId] FOREIGN KEY ([WarehouseId]) REFERENCES [Warehouse] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210501155149_Warehouses', N'3.1.7');

GO

ALTER TABLE [Vendor] ADD [VendorType] int NOT NULL DEFAULT 0;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210507134511_VendorType', N'3.1.7');

GO

DROP INDEX [IX_UnitConvert_InventItemId] ON [UnitConvert];

GO

DROP INDEX [IX_Size_InventItemId] ON [Size];

GO

DROP INDEX [IX_ShipmentContainer_PurchOrderShipmentHeaderId] ON [ShipmentContainer];

GO

DROP INDEX [IX_InventItemStoreConfiguration_InventItemId] ON [InventItemStoreConfiguration];

GO

DROP INDEX [IX_InventItem_Code] ON [InventItem];

GO

DROP INDEX [IX_InventDim_InventItemId] ON [InventDim];

GO

DROP INDEX [IX_Color_InventItemId] ON [Color];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Warehouse]') AND [c].[name] = N'Code');
IF @var61 IS NOT NULL EXEC(N'ALTER TABLE [Warehouse] DROP CONSTRAINT [' + @var61 + '];');
ALTER TABLE [Warehouse] ALTER COLUMN [Code] nvarchar(450) NULL;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Vendor]') AND [c].[name] = N'Code');
IF @var62 IS NOT NULL EXEC(N'ALTER TABLE [Vendor] DROP CONSTRAINT [' + @var62 + '];');
ALTER TABLE [Vendor] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [Vendor] ADD DEFAULT ('P' + Format(NEXT VALUE FOR dbo.Vendors,'D8')) FOR [Code];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[UserSettings]') AND [c].[name] = N'Username');
IF @var63 IS NOT NULL EXEC(N'ALTER TABLE [UserSettings] DROP CONSTRAINT [' + @var63 + '];');
ALTER TABLE [UserSettings] ALTER COLUMN [Username] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Unit]') AND [c].[name] = N'Code');
IF @var64 IS NOT NULL EXEC(N'ALTER TABLE [Unit] DROP CONSTRAINT [' + @var64 + '];');
ALTER TABLE [Unit] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TaxOnItem]') AND [c].[name] = N'Code');
IF @var65 IS NOT NULL EXEC(N'ALTER TABLE [TaxOnItem] DROP CONSTRAINT [' + @var65 + '];');
ALTER TABLE [TaxOnItem] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TaxItemGroupHeading]') AND [c].[name] = N'Code');
IF @var66 IS NOT NULL EXEC(N'ALTER TABLE [TaxItemGroupHeading] DROP CONSTRAINT [' + @var66 + '];');
ALTER TABLE [TaxItemGroupHeading] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Store]') AND [c].[name] = N'Code');
IF @var67 IS NOT NULL EXEC(N'ALTER TABLE [Store] DROP CONSTRAINT [' + @var67 + '];');
ALTER TABLE [Store] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Size]') AND [c].[name] = N'Code');
IF @var68 IS NOT NULL EXEC(N'ALTER TABLE [Size] DROP CONSTRAINT [' + @var68 + '];');
ALTER TABLE [Size] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShippingStepType]') AND [c].[name] = N'Code');
IF @var69 IS NOT NULL EXEC(N'ALTER TABLE [ShippingStepType] DROP CONSTRAINT [' + @var69 + '];');
ALTER TABLE [ShippingStepType] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShippingRouteStatusPresentationSchema]') AND [c].[name] = N'Description');
IF @var70 IS NOT NULL EXEC(N'ALTER TABLE [ShippingRouteStatusPresentationSchema] DROP CONSTRAINT [' + @var70 + '];');
ALTER TABLE [ShippingRouteStatusPresentationSchema] ALTER COLUMN [Description] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShippingRoute]') AND [c].[name] = N'Code');
IF @var71 IS NOT NULL EXEC(N'ALTER TABLE [ShippingRoute] DROP CONSTRAINT [' + @var71 + '];');
ALTER TABLE [ShippingRoute] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShippingCompany]') AND [c].[name] = N'Name');
IF @var72 IS NOT NULL EXEC(N'ALTER TABLE [ShippingCompany] DROP CONSTRAINT [' + @var72 + '];');
ALTER TABLE [ShippingCompany] ALTER COLUMN [Name] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShipmentTypes]') AND [c].[name] = N'Description');
IF @var73 IS NOT NULL EXEC(N'ALTER TABLE [ShipmentTypes] DROP CONSTRAINT [' + @var73 + '];');
ALTER TABLE [ShipmentTypes] ALTER COLUMN [Description] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShipmentImport]') AND [c].[name] = N'BLNumber');
IF @var74 IS NOT NULL EXEC(N'ALTER TABLE [ShipmentImport] DROP CONSTRAINT [' + @var74 + '];');
ALTER TABLE [ShipmentImport] ALTER COLUMN [BLNumber] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShipmentContainerType]') AND [c].[name] = N'Description');
IF @var75 IS NOT NULL EXEC(N'ALTER TABLE [ShipmentContainerType] DROP CONSTRAINT [' + @var75 + '];');
ALTER TABLE [ShipmentContainerType] ALTER COLUMN [Description] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ShipmentContainer]') AND [c].[name] = N'ContainerNumber');
IF @var76 IS NOT NULL EXEC(N'ALTER TABLE [ShipmentContainer] DROP CONSTRAINT [' + @var76 + '];');
ALTER TABLE [ShipmentContainer] ALTER COLUMN [ContainerNumber] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[SalesPriceDefinition]') AND [c].[name] = N'Code');
IF @var77 IS NOT NULL EXEC(N'ALTER TABLE [SalesPriceDefinition] DROP CONSTRAINT [' + @var77 + '];');
ALTER TABLE [SalesPriceDefinition] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationApprovalRuleCondition]') AND [c].[name] = N'Code');
IF @var78 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationApprovalRuleCondition] DROP CONSTRAINT [' + @var78 + '];');
ALTER TABLE [PurchQuotationApprovalRuleCondition] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationApprovalRule]') AND [c].[name] = N'Code');
IF @var79 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationApprovalRule] DROP CONSTRAINT [' + @var79 + '];');
ALTER TABLE [PurchQuotationApprovalRule] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationApproval]') AND [c].[name] = N'Code');
IF @var80 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationApproval] DROP CONSTRAINT [' + @var80 + '];');
ALTER TABLE [PurchQuotationApproval] ALTER COLUMN [Code] nvarchar(450) NULL;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotation]') AND [c].[name] = N'Code');
IF @var81 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotation] DROP CONSTRAINT [' + @var81 + '];');
ALTER TABLE [PurchQuotation] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [PurchQuotation] ADD DEFAULT ('RQ' + Format(NEXT VALUE FOR dbo.PurchQuotations,'D8')) FOR [Code];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchPaymentCondition]') AND [c].[name] = N'Code');
IF @var82 IS NOT NULL EXEC(N'ALTER TABLE [PurchPaymentCondition] DROP CONSTRAINT [' + @var82 + '];');
ALTER TABLE [PurchPaymentCondition] ALTER COLUMN [Code] nvarchar(450) NULL;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderShipmentHeader]') AND [c].[name] = N'Code');
IF @var83 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderShipmentHeader] DROP CONSTRAINT [' + @var83 + '];');
ALTER TABLE [PurchOrderShipmentHeader] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [PurchOrderShipmentHeader] ADD DEFAULT ('EC' + Format(NEXT VALUE FOR dbo.PurchOrderShipments,'D8')) FOR [Code];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderHeader]') AND [c].[name] = N'Code');
IF @var84 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderHeader] DROP CONSTRAINT [' + @var84 + '];');
ALTER TABLE [PurchOrderHeader] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [PurchOrderHeader] ADD DEFAULT ('OC' + Format(NEXT VALUE FOR dbo.PurchOrders,'D8')) FOR [Code];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Port]') AND [c].[name] = N'Code');
IF @var85 IS NOT NULL EXEC(N'ALTER TABLE [Port] DROP CONSTRAINT [' + @var85 + '];');
ALTER TABLE [Port] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemSeasonalCategory]') AND [c].[name] = N'Code');
IF @var86 IS NOT NULL EXEC(N'ALTER TABLE [ItemSeasonalCategory] DROP CONSTRAINT [' + @var86 + '];');
ALTER TABLE [ItemSeasonalCategory] ALTER COLUMN [Code] nvarchar(450) NULL;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemHierarchy]') AND [c].[name] = N'Code');
IF @var87 IS NOT NULL EXEC(N'ALTER TABLE [ItemHierarchy] DROP CONSTRAINT [' + @var87 + '];');
ALTER TABLE [ItemHierarchy] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemBarcode]') AND [c].[name] = N'Barcode');
IF @var88 IS NOT NULL EXEC(N'ALTER TABLE [ItemBarcode] DROP CONSTRAINT [' + @var88 + '];');
ALTER TABLE [ItemBarcode] ALTER COLUMN [Barcode] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventModelGroup]') AND [c].[name] = N'Code');
IF @var89 IS NOT NULL EXEC(N'ALTER TABLE [InventModelGroup] DROP CONSTRAINT [' + @var89 + '];');
ALTER TABLE [InventModelGroup] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventItemGroup]') AND [c].[name] = N'Code');
IF @var90 IS NOT NULL EXEC(N'ALTER TABLE [InventItemGroup] DROP CONSTRAINT [' + @var90 + '];');
ALTER TABLE [InventItemGroup] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[InventDimGroup]') AND [c].[name] = N'Code');
IF @var91 IS NOT NULL EXEC(N'ALTER TABLE [InventDimGroup] DROP CONSTRAINT [' + @var91 + '];');
ALTER TABLE [InventDimGroup] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Incoterm]') AND [c].[name] = N'Code');
IF @var92 IS NOT NULL EXEC(N'ALTER TABLE [Incoterm] DROP CONSTRAINT [' + @var92 + '];');
ALTER TABLE [Incoterm] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[EmployeeDiscount]') AND [c].[name] = N'Code');
IF @var93 IS NOT NULL EXEC(N'ALTER TABLE [EmployeeDiscount] DROP CONSTRAINT [' + @var93 + '];');
ALTER TABLE [EmployeeDiscount] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[CustomerPriceGroup]') AND [c].[name] = N'Code');
IF @var94 IS NOT NULL EXEC(N'ALTER TABLE [CustomerPriceGroup] DROP CONSTRAINT [' + @var94 + '];');
ALTER TABLE [CustomerPriceGroup] ALTER COLUMN [Code] nvarchar(450) NULL;
...
IF @var95 IS NOT NULL EXEC(N'ALTER TABLE [Currency] DROP CONSTRAINT [' + @var95 + '];');
ALTER TABLE [Currency] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Country]') AND [c].[name] = N'Code');
IF @var96 IS NOT NULL EXEC(N'ALTER TABLE [Country] DROP CONSTRAINT [' + @var96 + '];');
ALTER TABLE [Country] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[CommercialEvent]') AND [c].[name] = N'Description');
IF @var97 IS NOT NULL EXEC(N'ALTER TABLE [CommercialEvent] DROP CONSTRAINT [' + @var97 + '];');
ALTER TABLE [CommercialEvent] ALTER COLUMN [Description] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Color]') AND [c].[name] = N'Code');
IF @var98 IS NOT NULL EXEC(N'ALTER TABLE [Color] DROP CONSTRAINT [' + @var98 + '];');
ALTER TABLE [Color] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Brand]') AND [c].[name] = N'Code');
IF @var99 IS NOT NULL EXEC(N'ALTER TABLE [Brand] DROP CONSTRAINT [' + @var99 + '];');
ALTER TABLE [Brand] ALTER COLUMN [Code] nvarchar(450) NULL;
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Barcode]') AND [c].[name] = N'Code');
IF @var100 IS NOT NULL EXEC(N'ALTER TABLE [Barcode] DROP CONSTRAINT [' + @var100 + '];');
ALTER TABLE [Barcode] ALTER COLUMN [Code] nvarchar(450) NULL;

GO

CREATE UNIQUE INDEX [IX_Warehouse_Code] ON [Warehouse] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Vendor_Code] ON [Vendor] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_UserSettings_Username] ON [UserSettings] ([Username]) WHERE [Username] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_UnitConvert_InventItemId_FromUnitId_ToUnitId] ON [UnitConvert] ([InventItemId], [FromUnitId], [ToUnitId]);

GO

CREATE UNIQUE INDEX [IX_Unit_Code] ON [Unit] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_TaxOnItem_Code] ON [TaxOnItem] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_TaxItemGroupHeading_Code] ON [TaxItemGroupHeading] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Store_Code] ON [Store] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Size_InventItemId_Code] ON [Size] ([InventItemId], [Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShippingStepType_Code] ON [ShippingStepType] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShippingRouteStatusPresentationSchema_Description] ON [ShippingRouteStatusPresentationSchema] ([Description]) WHERE [Description] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShippingRoute_Code] ON [ShippingRoute] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShippingCompany_Name] ON [ShippingCompany] ([Name]) WHERE [Name] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShipmentTypes_Description] ON [ShipmentTypes] ([Description]) WHERE [Description] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShipmentImport_BLNumber] ON [ShipmentImport] ([BLNumber]) WHERE [BLNumber] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShipmentContainerType_Description] ON [ShipmentContainerType] ([Description]) WHERE [Description] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ShipmentContainer_PurchOrderShipmentHeaderId_ContainerNumber] ON [ShipmentContainer] ([PurchOrderShipmentHeaderId], [ContainerNumber]) WHERE [ContainerNumber] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_SalesPriceDefinition_Code] ON [SalesPriceDefinition] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchQuotationApprovalRuleCondition_Code] ON [PurchQuotationApprovalRuleCondition] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchQuotationApprovalRule_Code] ON [PurchQuotationApprovalRule] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchQuotationApproval_Code] ON [PurchQuotationApproval] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchQuotation_Code] ON [PurchQuotation] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchPaymentCondition_Code] ON [PurchPaymentCondition] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchOrderShipmentHeader_Code] ON [PurchOrderShipmentHeader] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_PurchOrderHeader_Code] ON [PurchOrderHeader] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Port_Code] ON [Port] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ItemSeasonalCategory_Code] ON [ItemSeasonalCategory] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ItemHierarchy_Code] ON [ItemHierarchy] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_ItemBarcode_Barcode] ON [ItemBarcode] ([Barcode]) WHERE [Barcode] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_InventModelGroup_Code] ON [InventModelGroup] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_InventItemStoreConfiguration_InventItemId_StoreId] ON [InventItemStoreConfiguration] ([InventItemId], [StoreId]);

GO

CREATE UNIQUE INDEX [IX_InventItemGroup_Code] ON [InventItemGroup] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_InventItem_Code] ON [InventItem] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_InventDimGroup_Code] ON [InventDimGroup] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_InventDim_InventItemId_ColorId_SizeId] ON [InventDim] ([InventItemId], [ColorId], [SizeId]) WHERE [ColorId] IS NOT NULL AND [SizeId] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Incoterm_Code] ON [Incoterm] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_EmployeeDiscount_Code] ON [EmployeeDiscount] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_CustomerPriceGroup_Code] ON [CustomerPriceGroup] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Currency_Code] ON [Currency] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Country_Code] ON [Country] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_CommercialEvent_Description] ON [CommercialEvent] ([Description]) WHERE [Description] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Color_InventItemId_Code] ON [Color] ([InventItemId], [Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Brand_Code] ON [Brand] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE UNIQUE INDEX [IX_Barcode_Code] ON [Barcode] ([Code]) WHERE [Code] IS NOT NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210508135540_PKsCode', N'3.1.7');

GO

DROP INDEX [IX_SalesPrice_InventItemId] ON [SalesPrice];

GO

CREATE UNIQUE INDEX [IX_SalesPrice_InventItemId_CustomerPriceGroupId] ON [SalesPrice] ([InventItemId], [CustomerPriceGroupId]);

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210512230652_UniqueSalesPrice', N'3.1.7');

GO

ALTER TABLE [SalesPriceDefinition] ADD [Reference] nvarchar(max) NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210519160045_ReferenceInSalesPriceDefinition', N'3.1.7');

GO

CREATE SEQUENCE [PurchCostDefinitions] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;

GO

CREATE SEQUENCE [SalesPriceDefinitions] START WITH 1000000 INCREMENT BY 1 NO MINVALUE NO MAXVALUE NO CYCLE;
...
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[SalesPriceDefinition]') AND [c].[name] = N'Code');
IF @var101 IS NOT NULL EXEC(N'ALTER TABLE [SalesPriceDefinition] DROP CONSTRAINT [' + @var101 + '];');
ALTER TABLE [SalesPriceDefinition] ALTER COLUMN [Code] nvarchar(450) NULL;
ALTER TABLE [SalesPriceDefinition] ADD DEFAULT ('DP' + Format(NEXT VALUE FOR dbo.SalesPriceDefinitions,'D8')) FOR [Code];

GO

ALTER TABLE [ItemBarcode] ADD [ERPVariantCode] nvarchar(max) NULL;

GO

CREATE TABLE [PurchCostDefinition] (
    [Id] bigint NOT NULL IDENTITY,
    [Code] nvarchar(450) NULL DEFAULT ('DC' + Format(NEXT VALUE FOR dbo.PurchCostDefinitions,'D8')),
    [Description] nvarchar(max) NULL,
    ...
    [ImportPolicyCode] nvarchar(max) NULL,
    [ShipmentImportCode] nvarchar(max) NULL,
    [ShippingCompanyCode] nvarchar(max) NULL,
    [ExchangeRate] decimal(18,2) NOT NULL,
    CONSTRAINT [PK_PurchCostDefinition] PRIMARY KEY ([Id])
);

GO

CREATE TABLE [PurchCostDefinitionLine] (
    [Id] bigint NOT NULL IDENTITY,
    [LineNumber] int NOT NULL,
    [QtyOrdered] decimal(18,2) NOT NULL,
    [UnitCode] nvarchar(max) NULL,
    [LineDescription] nvarchar(max) NULL,
    [CustomsTariffCode] nvarchar(max) NULL,
    [CustomsTariffPosition] nvarchar(max) NULL,
    [CBM] nvarchar(max) NULL,
    [EmptyField] nvarchar(max) NULL,
    [ItemNumber] nvarchar(max) NULL,
    [ItemBarcode] nvarchar(max) NULL,
    [InventItemCode] nvarchar(max) NULL,
    [PurchPrice] decimal(18,2) NOT NULL,
    [PurchLineAmount] decimal(18,2) NOT NULL,
    [CIFMarkupAmount] decimal(18,2) NOT NULL,
    [CostWithCIF] decimal(18,2) NOT NULL,
    [CostWithCIFLocalCurrency] decimal(18,2) NOT NULL,
    [CustomsFee] decimal(18,2) NOT NULL,
    [CustomsSelectiveFeeAmount] decimal(18,2) NOT NULL,
    [LocalExpensesMarkupAmount] decimal(18,2) NOT NULL,
    [CostWithLocalExpensesLocalCurrency] decimal(18,2) NOT NULL,
    [CostWithLocalExpenses] decimal(18,2) NOT NULL,
    [SuggestedSalesPriceStore] decimal(18,2) NOT NULL,
    [SuggestedSalesPriceWholesaleCredit] decimal(18,2) NOT NULL,
    [SuggestedSalesPriceWholesaleCash] decimal(18,2) NOT NULL,
    [PurchCostDefinitionId] bigint NOT NULL,
    CONSTRAINT [PK_PurchCostDefinitionLine] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_PurchCostDefinitionLine_PurchCostDefinition_PurchCostDefinitionId] FOREIGN KEY ([PurchCostDefinitionId]) REFERENCES [PurchCostDefinition] ([Id]) ON DELETE NO ACTION
);

GO

CREATE UNIQUE INDEX [IX_PurchCostDefinition_Code] ON [PurchCostDefinition] ([Code]) WHERE [Code] IS NOT NULL;

GO

CREATE INDEX [IX_PurchCostDefinitionLine_PurchCostDefinitionId] ON [PurchCostDefinitionLine] ([PurchCostDefinitionId]);

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210522203715_PurchCostDefinition', N'3.1.7');

GO

ALTER TABLE [Warehouse] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Warehouse] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [Warehouse] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Warehouse] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [SalesPriceDefinitionLine] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [SalesPriceDefinitionLine] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [SalesPriceDefinitionLine] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [SalesPriceDefinitionLine] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchPaymentCondition] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchPaymentCondition] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchPaymentCondition] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchPaymentCondition] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchCostDefinitionLine] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchCostDefinitionLine] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchCostDefinitionLine] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchCostDefinitionLine] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchCostDefinition] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchCostDefinition] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [PurchCostDefinition] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [PurchCostDefinition] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ItemSeasonalCategory] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [ItemSeasonalCategory] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ItemSeasonalCategory] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [ItemSeasonalCategory] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ItemBarcode] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [ItemBarcode] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ItemBarcode] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [ItemBarcode] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [CustomerPriceGroup] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [CustomerPriceGroup] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [CustomerPriceGroup] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [CustomerPriceGroup] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [Currency] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Currency] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [Currency] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Currency] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [BarcodeSource] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [BarcodeSource] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [BarcodeSource] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [BarcodeSource] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [BarcodeBatch] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [BarcodeBatch] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [BarcodeBatch] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [BarcodeBatch] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [Barcode] ADD [CreatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Barcode] ADD [CreatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [Barcode] ADD [UpdatedBy] nvarchar(max) NULL;

GO

ALTER TABLE [Barcode] ADD [UpdatedDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210526022446_SetAuditFields', N'3.1.7');

GO

ALTER TABLE [PurchCostDefinitionLine] ADD [PurchOrderCode] nvarchar(max) NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210527022025_PurchCodeInPurchCostDefLine', N'3.1.7');

GO

ALTER TABLE [PurchQuotation] ADD [PurchCommercialDepartmentId] bigint NULL;

GO

CREATE TABLE [InventItemPrepackBarcode] (
    [Id] bigint NOT NULL IDENTITY,
    [Qty] decimal(18,2) NOT NULL,
    [InventItemId] bigint NOT NULL,
    [ItemBarcodeId] bigint NOT NULL,
    CONSTRAINT [PK_InventItemPrepackBarcode] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_InventItemPrepackBarcode_InventItem_InventItemId] FOREIGN KEY ([InventItemId]) REFERENCES [InventItem] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_InventItemPrepackBarcode_ItemBarcode_ItemBarcodeId] FOREIGN KEY ([ItemBarcodeId]) REFERENCES [ItemBarcode] ([Id]) ON DELETE NO ACTION
);

GO

CREATE TABLE [PurchCommercialDepartment] (
    [Id] bigint NOT NULL IDENTITY,
    [CreatedDate] datetime2 NOT NULL,
    [CreatedBy] nvarchar(max) NULL,
    [UpdatedDate] datetime2 NOT NULL,
    ...
    [Code] nvarchar(max) NULL,
    [Description] nvarchar(max) NULL,
    CONSTRAINT [PK_PurchCommercialDepartment] PRIMARY KEY ([Id])
);

GO

CREATE TABLE [PurchOrderShipmentCrossDocking] (
    [Id] bigint NOT NULL IDENTITY,
    [Qty] decimal(18,2) NOT NULL,
    [PurchOrderShipmentDetailId] bigint NOT NULL,
    [WarehouseId] bigint NOT NULL,
    CONSTRAINT [PK_PurchOrderShipmentCrossDocking] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_PurchOrderShipmentCrossDocking_PurchOrderShipmentDetail_PurchOrderShipmentDetailId] FOREIGN KEY ([PurchOrderShipmentDetailId]) REFERENCES [PurchOrderShipmentDetail] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_PurchOrderShipmentCrossDocking_Warehouse_WarehouseId] FOREIGN KEY ([WarehouseId]) REFERENCES [Warehouse] ([Id]) ON DELETE NO ACTION
);

GO

CREATE TABLE [PurchRole] (
    [Id] bigint NOT NULL IDENTITY,
    [CreatedDate] datetime2 NOT NULL,
    [CreatedBy] nvarchar(max) NULL,
    [UpdatedDate] datetime2 NOT NULL,
    ...
    [Code] nvarchar(max) NULL,
    [Description] nvarchar(max) NULL,
    CONSTRAINT [PK_PurchRole] PRIMARY KEY ([Id])
);

GO

CREATE TABLE [PurchQuotationApprovalRole] (
    [Id] bigint NOT NULL IDENTITY,
    [CreatedDate] datetime2 NOT NULL,
    [CreatedBy] nvarchar(max) NULL,
    [UpdatedDate] datetime2 NOT NULL,
    [UpdatedBy] nvarchar(max) NULL,
    [PurchQuotationApprovalId] bigint NOT NULL,
    [PurchRoleId] bigint NOT NULL,
    [PurchCommercialDepartmentId] bigint NOT NULL,
    CONSTRAINT [PK_PurchQuotationApprovalRole] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_PurchQuotationApprovalRole_PurchCommercialDepartment_PurchCommercialDepartmentId] FOREIGN KEY ([PurchCommercialDepartmentId]) REFERENCES [PurchCommercialDepartment] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_PurchQuotationApprovalRole_PurchQuotationApproval_PurchQuotationApprovalId] FOREIGN KEY ([PurchQuotationApprovalId]) REFERENCES [PurchQuotationApproval] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_PurchQuotationApprovalRole_PurchRole_PurchRoleId] FOREIGN KEY ([PurchRoleId]) REFERENCES [PurchRole] ([Id]) ON DELETE NO ACTION
);

GO

CREATE TABLE [PurchQuotationApprovalRuleConditionStep] (
    [Id] bigint NOT NULL IDENTITY,
    [CreatedDate] datetime2 NOT NULL,
    [CreatedBy] nvarchar(max) NULL,
    [UpdatedDate] datetime2 NOT NULL,
    [UpdatedBy] nvarchar(max) NULL,
    [PurchQuotationApprovalRuleConditionId] bigint NOT NULL,
    [PurchRoleId] bigint NOT NULL,
    [Order] bigint NOT NULL,
    CONSTRAINT [PK_PurchQuotationApprovalRuleConditionStep] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_PurchQuotationApprovalRuleConditionStep_PurchQuotationApprovalRuleCondition_PurchQuotationApprovalRuleConditionId] FOREIGN KEY ([PurchQuotationApprovalRuleConditionId]) REFERENCES [PurchQuotationApprovalRuleCondition] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_PurchQuotationApprovalRuleConditionStep_PurchRole_PurchRoleId] FOREIGN KEY ([PurchRoleId]) REFERENCES [PurchRole] ([Id]) ON DELETE NO ACTION
);

GO

CREATE INDEX [IX_PurchQuotation_PurchCommercialDepartmentId] ON [PurchQuotation] ([PurchCommercialDepartmentId]);

GO

CREATE INDEX [IX_InventItemPrepackBarcode_InventItemId] ON [InventItemPrepackBarcode] ([InventItemId]);

GO

CREATE INDEX [IX_InventItemPrepackBarcode_ItemBarcodeId] ON [InventItemPrepackBarcode] ([ItemBarcodeId]);

GO

CREATE INDEX [IX_PurchOrderShipmentCrossDocking_PurchOrderShipmentDetailId] ON [PurchOrderShipmentCrossDocking] ([PurchOrderShipmentDetailId]);

GO

CREATE INDEX [IX_PurchOrderShipmentCrossDocking_WarehouseId] ON [PurchOrderShipmentCrossDocking] ([WarehouseId]);

GO

CREATE INDEX [IX_PurchQuotationApprovalRole_PurchCommercialDepartmentId] ON [PurchQuotationApprovalRole] ([PurchCommercialDepartmentId]);

GO

CREATE INDEX [IX_PurchQuotationApprovalRole_PurchQuotationApprovalId] ON [PurchQuotationApprovalRole] ([PurchQuotationApprovalId]);

GO

CREATE INDEX [IX_PurchQuotationApprovalRole_PurchRoleId] ON [PurchQuotationApprovalRole] ([PurchRoleId]);

GO

CREATE INDEX [IX_PurchQuotationApprovalRuleConditionStep_PurchQuotationApprovalRuleConditionId] ON [PurchQuotationApprovalRuleConditionStep] ([PurchQuotationApprovalRuleConditionId]);

GO

CREATE INDEX [IX_PurchQuotationApprovalRuleConditionStep_PurchRoleId] ON [PurchQuotationApprovalRuleConditionStep] ([PurchRoleId]);

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_PurchCommercialDepartment_PurchCommercialDepartmentId] FOREIGN KEY ([PurchCommercialDepartmentId]) REFERENCES [PurchCommercialDepartment] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210608134325_PurchApprovalEntities', N'3.1.7');

GO

ALTER TABLE [PurchQuotationApprovalLog] ADD [PurchQuotationApprovalAction] int NOT NULL DEFAULT 0;

GO

ALTER TABLE [ItemHierarchy] ADD [InventItemGroupId] bigint NULL;

GO

CREATE INDEX [IX_ItemHierarchy_InventItemGroupId] ON [ItemHierarchy] ([InventItemGroupId]);

GO

ALTER TABLE [ItemHierarchy] ADD CONSTRAINT [FK_ItemHierarchy_InventItemGroup_InventItemGroupId] FOREIGN KEY ([InventItemGroupId]) REFERENCES [InventItemGroup] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210612162003_ItemGroupInHierarchy', N'3.1.7');

GO

ALTER TABLE [PurchQuotationApprovalLog] ADD [PurchQuotationApprovalRuleConditionStepId] bigint NOT NULL DEFAULT CAST(0 AS bigint);

GO

ALTER TABLE [PurchQuotation] ADD [PurchQuotationApprovalRuleConditionId] bigint NULL;

GO

ALTER TABLE [PurchQuotation] ADD [PurchQuotationApprovalRuleConditionStepId] bigint NULL;

GO

CREATE INDEX [IX_PurchQuotationApprovalLog_PurchQuotationApprovalRuleConditionStepId] ON [PurchQuotationApprovalLog] ([PurchQuotationApprovalRuleConditionStepId]);

GO

CREATE INDEX [IX_PurchQuotation_PurchQuotationApprovalRuleConditionId] ON [PurchQuotation] ([PurchQuotationApprovalRuleConditionId]);

GO

CREATE INDEX [IX_PurchQuotation_PurchQuotationApprovalRuleConditionStepId] ON [PurchQuotation] ([PurchQuotationApprovalRuleConditionStepId]);

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_PurchQuotationApprovalRuleCondition_PurchQuotationApprovalRuleConditionId] FOREIGN KEY ([PurchQuotationApprovalRuleConditionId]) REFERENCES [PurchQuotationApprovalRuleCondition] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotation] ADD CONSTRAINT [FK_PurchQuotation_PurchQuotationApprovalRuleConditionStep_PurchQuotationApprovalRuleConditionStepId] FOREIGN KEY ([PurchQuotationApprovalRuleConditionStepId]) REFERENCES [PurchQuotationApprovalRuleConditionStep] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotationApprovalLog] ADD CONSTRAINT [FK_PurchQuotationApprovalLog_PurchQuotationApprovalRuleConditionStep_PurchQuotationApprovalRuleConditionStepId] FOREIGN KEY ([PurchQuotationApprovalRuleConditionStepId]) REFERENCES [PurchQuotationApprovalRuleConditionStep] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210612231548_ConditionAndStepInPurchQuotation', N'3.1.7');
...
DROP INDEX [IX_PurchQuotationApprovalRuleCondition_PurchQuotationApprovalId] ON [PurchQuotationApprovalRuleCondition];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationApprovalRuleCondition]') AND [c].[name] = N'PurchQuotationApprovalId');
IF @var102 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationApprovalRuleCondition] DROP CONSTRAINT [' + @var102 + '];');
ALTER TABLE [PurchQuotationApprovalRuleCondition] DROP COLUMN [PurchQuotationApprovalId];

GO

ALTER TABLE [PurchQuotationApprovalRuleCondition] ADD [IsActive] decimal(18,2) NOT NULL DEFAULT 0.0;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210615225839_FixApprovalCondition', N'3.1.7');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationApprovalRuleCondition]') AND [c].[name] = N'IsActive');
IF @var103 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationApprovalRuleCondition] DROP CONSTRAINT [' + @var103 + '];');
ALTER TABLE [PurchQuotationApprovalRuleCondition] ALTER COLUMN [IsActive] bit NOT NULL;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210615230106_FixApprovalCondition2', N'3.1.7');

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] DROP CONSTRAINT [FK_PurchOrderShipmentCrossDocking_Warehouse_WarehouseId];

GO

DROP INDEX [IX_PurchOrderShipmentCrossDocking_WarehouseId] ON [PurchOrderShipmentCrossDocking];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderShipmentCrossDocking]') AND [c].[name] = N'WarehouseId');
IF @var104 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderShipmentCrossDocking] DROP CONSTRAINT [' + @var104 + '];');
ALTER TABLE [PurchOrderShipmentCrossDocking] DROP COLUMN [WarehouseId];

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] ADD [StoreId] bigint NOT NULL DEFAULT CAST(0 AS bigint);

GO

CREATE TABLE [PurchQuotationLinePrepackDetail] (
    [Id] bigint NOT NULL IDENTITY,
    [PurchPrice] decimal(18,2) NOT NULL,
    [InventItemPrepackBarcodeId] bigint NOT NULL,
    [PurchQuotationLineId] bigint NOT NULL,
    CONSTRAINT [PK_PurchQuotationLinePrepackDetail] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_PurchQuotationLinePrepackDetail_InventItemPrepackBarcode_InventItemPrepackBarcodeId] FOREIGN KEY ([InventItemPrepackBarcodeId]) REFERENCES [InventItemPrepackBarcode] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_PurchQuotationLinePrepackDetail_PurchQuotationLine_PurchQuotationLineId] FOREIGN KEY ([PurchQuotationLineId]) REFERENCES [PurchQuotationLine] ([Id]) ON DELETE NO ACTION
);

GO

CREATE INDEX [IX_PurchOrderShipmentCrossDocking_StoreId] ON [PurchOrderShipmentCrossDocking] ([StoreId]);

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_InventItemPrepackBarcodeId] ON [PurchQuotationLinePrepackDetail] ([InventItemPrepackBarcodeId]);

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_PurchQuotationLineId] ON [PurchQuotationLinePrepackDetail] ([PurchQuotationLineId]);

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] ADD CONSTRAINT [FK_PurchOrderShipmentCrossDocking_Store_StoreId] FOREIGN KEY ([StoreId]) REFERENCES [Store] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210625130024_QuotationPrepackDetail', N'3.1.7');

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] DROP CONSTRAINT [FK_PurchOrderShipmentCrossDocking_PurchOrderShipmentDetail_PurchOrderShipmentDetailId];

GO

DROP INDEX [IX_PurchOrderShipmentCrossDocking_PurchOrderShipmentDetailId] ON [PurchOrderShipmentCrossDocking];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchOrderShipmentCrossDocking]') AND [c].[name] = N'PurchOrderShipmentDetailId');
IF @var105 IS NOT NULL EXEC(N'ALTER TABLE [PurchOrderShipmentCrossDocking] DROP CONSTRAINT [' + @var105 + '];');
ALTER TABLE [PurchOrderShipmentCrossDocking] DROP COLUMN [PurchOrderShipmentDetailId];

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] ADD [ShipmentContainerDetailId] bigint NOT NULL DEFAULT CAST(0 AS bigint);

GO

CREATE INDEX [IX_PurchOrderShipmentCrossDocking_ShipmentContainerDetailId] ON [PurchOrderShipmentCrossDocking] ([ShipmentContainerDetailId]);

GO

ALTER TABLE [PurchOrderShipmentCrossDocking] ADD CONSTRAINT [FK_PurchOrderShipmentCrossDocking_ShipmentContainerDetail_ShipmentContainerDetailId] FOREIGN KEY ([ShipmentContainerDetailId]) REFERENCES [ShipmentContainerDetail] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210629015423_CrossDockingContainer', N'3.1.7');

GO

ALTER TABLE [ShipmentContainer] ADD [EquipmentPickupDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ShipmentContainer] ADD [EstimatedGateUnloadDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ShipmentContainer] ADD [RealGateUnloadDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ShipmentContainer] ADD [ToShippingCompanyNotificationDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210701003849_DatesOnShipmentContainer', N'3.1.7');

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210710144253_FreeTimeDatesInContainers', N'3.1.7');

GO

ALTER TABLE [ShipmentContainer] ADD [FreeTimeBoxLimitDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

ALTER TABLE [ShipmentContainer] ADD [FreeTimeChassisLimitDate] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210710144535_FreeTimeDatesInContainers2', N'3.1.7');

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] DROP CONSTRAINT [FK_PurchQuotationLinePrepackDetail_InventItemPrepackBarcode_InventItemPrepackBarcodeId];

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] DROP CONSTRAINT [FK_PurchQuotationLinePrepackDetail_PurchQuotationLine_PurchQuotationLineId];

GO

DROP INDEX [IX_PurchQuotationLinePrepackDetail_InventItemPrepackBarcodeId] ON [PurchQuotationLinePrepackDetail];

GO

DROP INDEX [IX_PurchOrderShipmentCrossDocking_ShipmentContainerDetailId] ON [PurchOrderShipmentCrossDocking];

GO

DROP INDEX [IX_InventItemPrepackBarcode_InventItemId] ON [InventItemPrepackBarcode];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PurchQuotationLinePrepackDetail]') AND [c].[name] = N'InventItemPrepackBarcodeId');
IF @var106 IS NOT NULL EXEC(N'ALTER TABLE [PurchQuotationLinePrepackDetail] DROP CONSTRAINT [' + @var106 + '];');
ALTER TABLE [PurchQuotationLinePrepackDetail] DROP COLUMN [InventItemPrepackBarcodeId];

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD [ColorId] bigint NULL;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD [InventItemId] bigint NOT NULL DEFAULT CAST(0 AS bigint);

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD [ItemBarcodeId] bigint NULL;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD [QtyPerPrepack] decimal(18,2) NOT NULL DEFAULT 0.0;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD [SizeId] bigint NULL;

GO

ALTER TABLE [InventItem] ADD [PurchPrice] decimal(18,2) NOT NULL DEFAULT 0.0;

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_ColorId] ON [PurchQuotationLinePrepackDetail] ([ColorId]);

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_InventItemId] ON [PurchQuotationLinePrepackDetail] ([InventItemId]);

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_ItemBarcodeId] ON [PurchQuotationLinePrepackDetail] ([ItemBarcodeId]);

GO

CREATE INDEX [IX_PurchQuotationLinePrepackDetail_SizeId] ON [PurchQuotationLinePrepackDetail] ([SizeId]);

GO

CREATE UNIQUE INDEX [IX_PurchOrderShipmentCrossDocking_ShipmentContainerDetailId_StoreId] ON [PurchOrderShipmentCrossDocking] ([ShipmentContainerDetailId], [StoreId]);

GO

CREATE UNIQUE INDEX [IX_InventItemPrepackBarcode_InventItemId_ItemBarcodeId] ON [InventItemPrepackBarcode] ([InventItemId], [ItemBarcodeId]);

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD CONSTRAINT [FK_PurchQuotationLinePrepackDetail_Color_ColorId] FOREIGN KEY ([ColorId]) REFERENCES [Color] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD CONSTRAINT [FK_PurchQuotationLinePrepackDetail_InventItem_InventItemId] FOREIGN KEY ([InventItemId]) REFERENCES [InventItem] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD CONSTRAINT [FK_PurchQuotationLinePrepackDetail_ItemBarcode_ItemBarcodeId] FOREIGN KEY ([ItemBarcodeId]) REFERENCES [ItemBarcode] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD CONSTRAINT [FK_PurchQuotationLinePrepackDetail_PurchQuotationLine_PurchQuotationLineId] FOREIGN KEY ([PurchQuotationLineId]) REFERENCES [PurchQuotationLine] ([Id]) ON DELETE CASCADE;

GO

ALTER TABLE [PurchQuotationLinePrepackDetail] ADD CONSTRAINT [FK_PurchQuotationLinePrepackDetail_Size_SizeId] FOREIGN KEY ([SizeId]) REFERENCES [Size] ([Id]) ON DELETE NO ACTION;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210713134227_PrepackkQuotationLine', N'3.1.7');
